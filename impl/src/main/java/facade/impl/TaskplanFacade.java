...
public class TaskplanFacade implements ITaskplanFacade {
    ...
    public void updateOne(TaskPlan taskPlan) {
        ...
        if(taskplanFromDbs!=null){
            if(!Objects.equals(taskPlan.getName(), "") && taskPlan.getName() !=null){
                taskplanFromDbs.setName(taskPlan.getName());
            }
            if(!Objects.equals(taskPlan.getDescription(), "") && taskPlan.getDescription() !=null){
    ...
    @Override
    public Set<TaskPlanView> getProjectUnAssignedTaskPlans(Long userId) {
        List<TaskPlan> projectUnAssignedTaskPlans=taskplanDao.getProjectUnAssignedTaskPlans(userId);
        return projectUnAssignedTaskPlans.stream().map(taskPlan -> taskPlanConverter.convert(taskPlan)).collect(Collectors.toSet());
    }

    @Override
    public void createOne(TaskPlan taskPlan) {
